...
speed_def = 30
params = {30:[300,250,325,10],40:[800,200,297,15]}#white threshold , x_offset , y_start, y offset (x_start = (width-x_off)/2)
def rand_color():
...
def detect_stopline(cal_image, low_threshold_value):
    ...
    image_lex = light_extract.change(stopline_roi)
    image_count = image_processing(image_lex, low_threshold_value)
    count = cv2.countNonZero(image_count)
    #is_stop = False
    if count > white_th:
        print("stopline")
        is_stop = True
...
def image_processing(image, low_threshold_value):
    ...
    _, L = cv2.threshold(L, low_threshold_value, 255, cv2.THRESH_BINARY)
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        #pos, frame = process_image(cam_image)
